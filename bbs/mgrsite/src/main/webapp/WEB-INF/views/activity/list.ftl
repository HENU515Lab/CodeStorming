                            ...
                            <table id="activities" class="table table-bordered table-striped">
                                ...
                                <#list activityList as activity>
                                    <tr>
                                        ...
                                        <td>${activity.activityAbstract}</td>
                                        <td>${activity.joinNum}</td>
                                        <td>${activity.startTime?string('yyyy-MM-dd hh:mm:ss')}</td>
                                        <td>${activity.endTime?string('yyyy-MM-dd hh:mm:ss')}</td>>
                                        <td><#if activity.detail?length lt 20>${(activity.detail)?default("")}
                                            <#else>${(activity.detail[0..10]?default(""))}...</#if></td>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-warning" data-toggle="modal"
                                                    data-target="#edit-modal" onclick="itemEdit(${activity.id})">
                                                编辑
                                            </button>
                                            <button type="button" class="btn btn-sm btn-danger" data-toggle="modal"
                                                    data-target="#remove-modal" onclick="itemRemove(${activity.id})">
                                                删除
                                            </button>
                                        </td>
                                    </tr>
